    ...
    private void findReferences(URL url, JsonNode document) throws ResolutionException {
        ...
        for (JsonNode refNode : referencePaths) {
            ...
            if (refInValidLocation.isEmpty()) {
                ...
                List<JsonNode> discriminators = document.findParents("discriminator");
                if (discriminators.stream()
                        .map(d -> d.get("discriminator"))
                        .filter(d -> d.has("mapping"))
                        .flatMap(d -> d.get("mapping").valueStream())
                        .noneMatch(m -> m.textValue().equals(schemaName))) {
                    continue;
                }
            }
